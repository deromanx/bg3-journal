...
def gemini_extract_praise(sid, text):
    prompt = f"""你是柏德之門3跑團日誌的分析員。從以下日誌中找出所有「稱讚/讚美/誇獎」事件，以純JSON陣列回傳（不加說明或markdown）。

「稱讚」的定義：
- 任何情景下（戰鬥中、日常對話、裝備討論等），對隊友的行為、表現、決策、個性給予真誠的正面評價
- 包括：「你這個做得很好」、「這波操作超強」、「幸虧有你」、「你真的太穩了」等
- 包括：發自內心的驚嘆讚美（「哇你怎麼這麼聰明」）
- 不包括：諷刺性稱讚、嘲諷中夾帶的假誇獎、靠北吐槽中帶嘲諷的假讚美
