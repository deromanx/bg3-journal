...
- deaths **不包含**玩家角色之間互相攻擊造成的陣亡/倒地——無論是正式決鬥、單挑、營地PK，
  或任何一方主動攻擊隊友（含蓄意整人、報復性反擊）導致的死亡，一律不列入 deaths；
  這類戰績只反映在 duels 欄位。只有死於 NPC、怪物、環境傷害（陷阱、墜落、自己的法術反彈等）
  或協同隊友對抗 NPC 時的意外（如隊友丟你去攻擊敵人時摔死）才算 deaths
- duels 只記錄玩家角色之間**真正的 PvP 戰鬥**（雙方互相攻擊、直到一方倒地或認輸），不含對 NPC 或怪物的戰鬥；
  也不含任何「用擲骰決定某件事」的非戰鬥場合——例如決定隊長、決定誰去買東西、猜拳、比大小、
  骰點數高低決勝負等。這類「A 骰出 X 點打敗/贏過 B」的敘述若沒有實際攻防過程，禁止列入 duels
